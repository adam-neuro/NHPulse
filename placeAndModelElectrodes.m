...
if length(elecPara)==1
    ...
    if isfield(elecPara,'elecGelSize') && ~isempty(elecPara.elecGelSize) && size(elecPara.elecGelSize,1)>1
        for i=1:length(temp), temp(i).elecGelSize = temp(i).elecGelSize(i,:); end
    end
    if isfield(elecPara,'elecSkinGap') && ~isempty(elecPara.elecSkinGap) && numel(elecPara.elecSkinGap)>1
        elecSkinGap = elecPara.elecSkinGap(:);
        for i=1:length(temp), temp(i).elecSkinGap = elecSkinGap(i); end
    end
    elecPara = temp;
end
...
for i = 1:length(elecPara) % size(elecLoc,1)    
    ...
    switch lower(elecPara(i).elecType)
        ...
        case 'disc'
            ...
            den = 2; % 2 points per pixel
            if isfield(elecPara(i),'elecGelSize') && ~isempty(elecPara(i).elecGelSize)
                gel_radius = elecPara(i).elecGelSize(1)/res;
                gel_height = elecPara(i).elecGelSize(2)/res;
                skin_gap = 0;
                if isfield(elecPara(i),'elecSkinGap') && ~isempty(elecPara(i).elecSkinGap)
                    skin_gap = elecPara(i).elecSkinGap/res;
                end

                % Custom disc gel geometry: gel contacts the scalp and
                % surrounds the lower portion of a smaller pin electrode.
                gelInset = min(0.5/res, gel_height/2);
                gel_in = elecLoc(i,:) - gelInset*normal;
                gel_out = elecLoc(i,:) + gel_height*normal;
                elec_in = elecLoc(i,:) + skin_gap*normal;
                electrode = elec_in + disc_height*normal;

                gel_coor = drawCylinder(0,gel_radius,gel_in,gel_out,den);
                elec_coor = drawCylinder(0,disc_radius,elec_in,electrode,den);
            else
