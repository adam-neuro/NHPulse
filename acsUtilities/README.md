...
Once the combined layout has been accepted, plan reinforced Velcro attachment
loops and build capMaker manufacturing STLs directly from those final
print-frame electrode coordinates:

```matlab
velcroPlan = acsPlanVelcroAnchors(combinedLayout, ...
    'earExclusionFile', activeEarExclusionFile, ...
    'outputFile', fullfile(pwd, 'outputs', 'debug', ...
        'M2107_tes8_eeg8_velcroAnchors.mat'), ...
    'editMode', 'always', ...
    'showFigures', true);
...
folder. The recommended retention workflow uses `strapMode='none'` with
`velcroAnchorMode='file'` so the cap includes reinforced loops for external
hook-and-loop straps. `acsPlanVelcroAnchors` opens with six automatic anchors:
caudolateral, preauricular, and rostrolateral on both sides. Shift-click a
visible scalp vertex to move the active loop; the saved plan carries both the
anchor coordinates and loop geometry used by the STL build. A saved plan is
reused only when the same output file is requested, the source mesh fingerprint
...
supports are selected to spread around each holder by at least
`holderSupportMinAngleDeg` when possible; `holderBridgeMode='auto'` adds
holder-to-holder stabilizer struts for holders whose rail-only supports still
approach from a narrow angle.

Printed chin straps remain available as a legacy option when explicitly needed.
`strapMode='earRostral'` estimates lateral strap anchors from the rostral edge
of the saved ear exclusion spheres; `strapMode='bboxLateral'` recovers the
older bounding-box heuristic. Legacy chin straps use
`strapCorrStyle='rectilinear'` by default, which voxelizes the strap as an
explicit square-wave sequence of vertical and horizontal bars. Set
`strapCorrStyle='swept'` only to recover the older swept-prism strap body.
...
For reference, the final manufacturing call with a previously saved Velcro plan
has this minimal form:
