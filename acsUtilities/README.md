...
If the cap should instead use external hook-and-loop straps, plan reinforced
edge loops before the final manufacturing build:
...
`acsPlanVelcroAnchors` opens with six automatic anchors: caudolateral,
preauricular, and rostrolateral on both sides. Shift-click a visible scalp
vertex to move the active loop; the saved plan carries both the anchor
coordinates and loop geometry used by the STL build.
