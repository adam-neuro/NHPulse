...
if isfield(optRoast,'segMaskFile') && ~isempty(optRoast.segMaskFile)
    masksFile = optRoast.segMaskFile;
end
...
extraTissues = [];
if isfield(optRoast,'extraTissues')
    extraTissues = optRoast.extraTissues;
end
...
if isRoast
    ...
    landmarksForDisplay = optRoast.landmarks;
    if isfield(optRoast.elecPara, 'indP') && isfield(optRoast.elecPara, 'indN') && ...
            isfield(optRoast.elecPara, 'indC') && ...
            isempty(optRoast.elecPara(1).indP) && isempty(optRoast.elecPara(1).indN) && ...
            ~isempty(optRoast.elecPara(1).indC)
        landmarksForDisplay = [];
    end
    viewElectrodes(mask,elec,gel,landmarksForDisplay,image,simTag);
