...
switch fun
    ...
    case 'roast'
        ...
        if isfield(opt,'leadField') && ~isempty(opt.leadField)
            fprintf(fid,'leadFieldMode:\t%s\n',opt.leadField.mode);
            fprintf(fid,'leadFieldReference:\t%s\n',opt.leadField.referenceElectrode);
            fprintf(fid,'leadFieldIncludesPassiveElectrodes:\t%d\n',opt.leadField.includePassiveElectrodes);
            fprintf(fid,'leadFieldElectrodes:\t');
            for i=1:length(opt.leadField.electrodeNames)
                fprintf(fid,'%s',opt.leadField.electrodeNames{i});
                if i<length(opt.leadField.electrodeNames), fprintf(fid,', '); end
            end
            fprintf(fid,'\n');
        end
        ...
        if isfield(opt.elecPara,'elecGelSize') && ~isempty(opt.elecPara(1).elecGelSize)
            fprintf(fid,'elecGelSize:\t');
            if length(opt.elecPara)==1
                for i=1:size(opt.elecPara.elecGelSize,1)
                    fprintf(fid,'[');
                    for j=1:size(opt.elecPara.elecGelSize,2)
                        fprintf(fid,'%.1f',opt.elecPara.elecGelSize(i,j));
                        if j<size(opt.elecPara.elecGelSize,2), fprintf(fid,','); end
                    end
                    fprintf(fid,']');
                    if i<size(opt.elecPara.elecGelSize,1), fprintf(fid,'; '); end
                end
            else
                for i=1:length(opt.elecPara)
                    fprintf(fid,'[');
                    for j=1:size(opt.elecPara(i).elecGelSize,2)
                        fprintf(fid,'%.1f',opt.elecPara(i).elecGelSize(j));
                        if j<size(opt.elecPara(i).elecGelSize,2), fprintf(fid,','); end
                    end
                    fprintf(fid,']');
                    if i<length(opt.elecPara), fprintf(fid,'; '); end
                end
        ...
        if isfield(opt.elecPara,'elecSkinGap') && ~isempty(opt.elecPara(1).elecSkinGap)
            fprintf(fid,'elecSkinGap:\t');
            if length(opt.elecPara)==1
                for i=1:numel(opt.elecPara.elecSkinGap)
                    fprintf(fid,'%.1f',opt.elecPara.elecSkinGap(i));
                    if i<numel(opt.elecPara.elecSkinGap), fprintf(fid,'; '); end
                end
            else
                for i=1:length(opt.elecPara)
                    fprintf(fid,'%.1f',opt.elecPara(i).elecSkinGap);
                    if i<length(opt.elecPara), fprintf(fid,'; '); end
                end
            end
            fprintf(fid,'\n');
        ...
        if isfield(opt,'extraTissues') && ~isempty(opt.extraTissues)
            tissueCfg = roastTissueConfig(opt.extraTissues);
            for iExtra = 1:numel(tissueCfg.extraTissues)
                fieldName = tissueCfg.extraTissues(iExtra).conductivityField;
                if isfield(opt.conductivities,fieldName)
                    fprintf(fid,'%s: %.6g; ',fieldName,opt.conductivities.(fieldName));
                end
            end
        end
        fprintf(fid,'\n');

        if isfield(opt,'segMaskFile') && ~isempty(opt.segMaskFile)
            fprintf(fid,'segMaskFile:\t%s\n',opt.segMaskFile);
        end
        if isfield(opt,'extraTissues') && ~isempty(opt.extraTissues)
            tissueCfg = roastTissueConfig(opt.extraTissues);
            fprintf(fid,'extraTissues:\t');
            for iExtra = 1:numel(tissueCfg.extraTissues)
                fprintf(fid,'%d=%s', ...
                    tissueCfg.extraTissues(iExtra).label, ...
                    tissueCfg.extraTissues(iExtra).name);
                if iExtra < numel(tissueCfg.extraTissues)
                    fprintf(fid,'; ');
                end
            end
            fprintf(fid,'\n');
