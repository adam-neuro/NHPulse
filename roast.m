...
while indArg <= length(varargin)
    switch lower(varargin{indArg})
        ...
        case 'elecsize'
            ...
            indArg = indArg+2;
        case 'elecgelsize'
            elecGelSize = varargin{indArg+1};
            indArg = indArg+2;
        case 'elecskingap'
            elecSkinGap = varargin{indArg+1};
            indArg = indArg+2;
        case 'elecori'
        ...
        case 'conductivities'
            ...
            indArg = indArg+2;
        case 'segmaskfile'
            segMaskFile = varargin{indArg+1};
            indArg = indArg+2;
        case 'extratissues'
            extraTissues = varargin{indArg+1};
            indArg = indArg+2;
        case 'simulationtag'
        ...
        case 'zeropadding'
            ...
            indArg = indArg+2;
        case 'leadfieldelectrodes'
            leadFieldElectrodes = varargin{indArg+1};
            indArg = indArg+2;
        case 'leadfieldreference'
            leadFieldReference = varargin{indArg+1};
            indArg = indArg+2;
        case 'electricfieldclimmode'
            electricFieldClimMode = varargin{indArg+1};
            indArg = indArg+2;
        case 'electricfieldclimpercentile'
            electricFieldClimPercentile = varargin{indArg+1};
            indArg = indArg+2;
        case 'electricfieldclim'
...
if ~exist('electricFieldClimMode','var') || isempty(electricFieldClimMode)
    electricFieldClimMode = 'surfacePercentile';
end
if ~exist('electricFieldClimPercentile','var') || isempty(electricFieldClimPercentile)
    electricFieldClimPercentile = 95;
end
...
visualizationOptions = struct( ...
    ...
    'electricFieldClimPercentile', electricFieldClimPercentile, ...
    'electricFieldClim', electricFieldClim);
...
if any(~strcmpi(recipe,'leadfield'))
    ...
    if ~exist('elecGelSize','var')
        elecGelSize = [];
    elseif ~isempty(elecGelSize)
        if ~isnumeric(elecGelSize) || any(elecGelSize(:)<=0) || size(elecGelSize,2)~=2
            error('Unrecognized electrode gel sizes. Please specify as [radius height] for the optional disc gel pool.');
        end
        if size(elecGelSize,1)>1 && size(elecGelSize,1)~=length(elecName)
            error('You want different gel sizes for each electrode. Please tell ROAST the gel size for each electrode respectively, in a N-row matrix, where N is the number of electrodes to be placed.');
        end
    end
    if ~exist('elecSkinGap','var')
        elecSkinGap = [];
    elseif ~isempty(elecSkinGap)
        if ~isnumeric(elecSkinGap) || any(elecSkinGap(:)<0) || ~ismember(numel(elecSkinGap),[1 length(elecName)])
            error('Unrecognized electrode skin gap. Please specify a non-negative scalar or one value per electrode.');
        end
        elecSkinGap = elecSkinGap(:);
    end

    elecPara = struct('capType',capType,'elecType',elecType,...
        'elecSize',elecSize,'elecOri',elecOri,...
        'elecGelSize',elecGelSize,'elecSkinGap',elecSkinGap);
    
else
    
    hasLeadFieldElectrodeGeometry = exist('elecType','var') || ...
        exist('elecSize','var') || exist('elecOri','var') || ...
        exist('elecGelSize','var') || exist('elecSkinGap','var');
    if hasLeadFieldElectrodeGeometry
        warning('You specified the ''recipe'' as the ''lead field generation''. Nice choice! ROAST will use the supplied electrode geometry options for the lead-field candidates. Note this will usually take a long time (>1 day) to generate the lead field for all the candidate electrodes.');
    else
    ...
    if exist('leadFieldElectrodes','var') && ~isempty(leadFieldElectrodes)
        if ischar(leadFieldElectrodes)
            elecName = {leadFieldElectrodes};
        elseif isstring(leadFieldElectrodes)
            elecName = cellstr(leadFieldElectrodes(:));
        elseif iscellstr(leadFieldElectrodes)
            elecName = leadFieldElectrodes(:);
        else
            error('Option ''leadFieldElectrodes'' must be a cell array or string array of electrode names.');
        end
        if length(elecName)<2
            error('Lead field generation requires at least two electrodes, including a reference electrode.');
        end
        if ~exist('leadFieldReference','var') || isempty(leadFieldReference)
            leadFieldReference = elecName{end};
        end
        leadFieldMode = 'custom';
    else
        ...
        if ~exist('leadFieldReference','var') || isempty(leadFieldReference)
            leadFieldReference = 'Iz';
        end
        leadFieldMode = 'legacy1010';
    end
    if isstring(leadFieldReference) && isscalar(leadFieldReference)
        leadFieldReference = char(leadFieldReference);
    end
    if ~ischar(leadFieldReference)
        error('Option ''leadFieldReference'' must be one electrode name.');
    end
    capType = '1010';
    if ~exist('elecType','var') || isempty(elecType)
        elecType = 'disc';
    end
    if ~exist('elecSize','var') || isempty(elecSize)
        if ~iscellstr(elecType)
            switch lower(elecType)
                case {'disc'}
                    elecSize = [6 2];
    ...
    if ~exist('elecOri','var') || isempty(elecOri)
        elecOri = [];
    end
    if ~exist('elecGelSize','var')
        elecGelSize = [];
...
if ~exist('extraTissues','var')
    extraTissues = [];
end
tissueCfg = roastTissueConfig(extraTissues);
extraTissues = tissueCfg.extraTissues;
...
if ~exist('conductivities','var')
    ...
    allowedConductivityNames = [{'white';'gray';'csf';'bone';'skin';'air';'gel';'electrode'}; ...
        tissueCfg.conductivityFields(7:end)'];
    if isempty(conductivitiesNam) || ~all(ismember(conductivitiesNam,allowedConductivityNames))
        error('Unrecognized tissue names detected in the conductivity option. Base supported names are ''white'', ''gray'', ''csf'', ''bone'', ''skin'', ''air'', ''gel'' and ''electrode''; extraTissues may add additional names such as ''titanium''.');
    end
...
conductivities = roastValidateExtraTissueConductivities(conductivities, tissueCfg);
...
if any(~strcmpi(recipe,'leadfield'))
    ...
    indRef = find(strcmpi(leadFieldReference,elecName),1);
    if isempty(indRef)
        error('Lead field reference electrode ''%s'' was not found among the candidate electrodes.', leadFieldReference);
    end
    leadField = struct();
    leadField.mode = leadFieldMode;
    leadField.electrodeNames = elecName(:);
    leadField.referenceElectrode = elecName{indRef};
    leadField.stimulusElectrodeNames = elecName(setdiff(1:length(elecName),indRef));
    leadField.includePassiveElectrodes = strcmpi(leadFieldMode,'custom');
    if strcmpi(leadFieldMode,'custom')
        leadField.customLocationFingerprint = roastCustomLocationFingerprint( ...
            [dirname filesep subjName '_customLocations'], elecName);
    else
        leadField.customLocationFingerprint = '';
    end
...
defaultSegMaskFile = [dirname filesep subjModelNameAftSeg '_masks.nii'];
if ~exist('segMaskFile','var') || isempty(segMaskFile)
    segMaskFileForRoast = defaultSegMaskFile;
    useCustomSegMask = false;
else
    segMaskFileForRoast = char(segMaskFile);
    useCustomSegMask = true;
    if exist(segMaskFileForRoast,'file') ~= 2
        error('The specified segMaskFile does not exist: %s', segMaskFileForRoast);
    end
end
...
if ~strcmp(subjName,'nyhead') && ~customSegContextReady
    if ~multiaxial
        ...
        if useCustomSegMask
            disp('======================================================')
            disp('      CUSTOM SEGMENTATION MASK PROVIDED, SKIP STEP 2  ')
            disp('======================================================')
        elseif ~exist(defaultSegMaskFile, 'file')
            disp('======================================================')
        ...
        if useCustomSegMask
            disp('======================================================')
            disp('      CUSTOM SEGMENTATION MASK PROVIDED, SKIP STEP 1  ')
            disp('======================================================')
        elseif ~exist(defaultSegMaskFile, 'file')
            disp('======================================================')
...
if ~exist('leadField','var'), leadField = []; end
segMaskInfo = dir(segMaskFileForRoast);
segMaskFingerprint = struct('file', segMaskFileForRoast, ...
    'bytes', segMaskInfo.bytes, ...
    'datenum', segMaskInfo.datenum, ...
    'date', segMaskInfo.date);
options = struct('configTxt',configTxt,'elecPara',elecPara,'subjRasRSPD',subjRasRSPD,'T2',T2,'multiaxial',multiaxial,'Affine',Affine,'mri2mni',mri2mni,'landmarks',landmarks,'meshOpt',meshOpt,'conductivities',conductivities,'uniqueTag',simTag,'resamp',doResamp,'zeroPad',paddingAmt,'isNonRAS',isNonRAS,'leadField',leadField,'segMaskFile',segMaskFileForRoast,'segMaskFingerprint',segMaskFingerprint,'extraTissues',extraTissues,'visualizationOptions',visualizationOptions);
...
landmarksForDisplay = landmarks;
if isempty(elecPara(1).indP) && isempty(elecPara(1).indN) && ~isempty(elecPara(1).indC)
    landmarksForDisplay = [];
...
if ~exist([dirname filesep subjName '_' uniqueTag '.mat'],'file')
    ...
    disp('======================================================')
    [node,elem,face] = meshByIso2mesh(subj,segMask,elec,gel,meshOpt,image,uniqueTag,extraTissues);
else
    disp('======================================================')
...
if any(~strcmpi(recipe,'leadfield'))
    ...
    if ~exist([dirname filesep subjName '_' uniqueTag '_v.pos'],'file')
        ...
        disp('======================================================')
        prepareForGetDP(subj,node,elem,elecName,uniqueTag,extraTissues);
        indElecSolve = 1:length(elecName);
        solveByGetDP(subj,injectCurrent,conductivities,indElecSolve,uniqueTag,'',extraTissues);
    else
        disp('======================================================')
    ...
    [~,indRef] = ismember(leadField.referenceElectrode,elecName);
    ...
    if any(~isSolved) && ~exist([dirname filesep subjName '_' uniqueTag '_roastResult.mat'],'file')
        ...
        for i=1:length(indStimElec)
            if ~isSolved(i)
                ...
                fprintf('======================================================\n\n');
                injectCurrent = zeros(length(elecName),1);
                injectCurrent(indStimElec(i)) = 1;
                injectCurrent(indRef) = -1;
                if leadField.includePassiveElectrodes
                    indElecSolve = 1:length(elecName);
                else
                    indElecSolve = [indStimElec(i) indRef];
                end
                solveByGetDP(subj,injectCurrent,conductivities,indElecSolve,uniqueTag,num2str(indStimElec(i)),extraTissues);
            else
