...
By default, ROAST retains the original 10-10 candidate layout for lead-field generation. Advanced workflows can instead provide a custom candidate list with `'leadFieldElectrodes'` and choose one of those candidates as the mathematical reference with `'leadFieldReference'`. Customized locations must follow the same `<subject>_customLocations` format used for ordinary simulations. This reuses the existing lead-field solver and preserves the legacy behavior when the new options are omitted.
